

############# file paths #############

#path to abcd Restricted Access folder
main_abcd_path = "~/Box Sync"
abcd_box_path = "2. Barzi Lab - Restricted Access/2-ABCD"
abcd_data_path = "ABCD data"
prs_box_path = "Ran_Barzilay/"
abcd_version = "4.0/"
abcd_covid = "covid19"
p_factor_box_path = "Projects/exposome/3.0/data/p factor scores/"
e_factor_box_path = "Projects/exposome/3.0/data/e factor scores/"

abcd_files_path = file.path(main_abcd_path, abcd_box_path, abcd_data_path, abcd_version)
abcd_covid_r1_files_path = file.path(main_abcd_path, abcd_box_path, abcd_data_path, abcd_covid, "1_Data_Release/")
abcd_covid_r2_files_path = file.path(main_abcd_path, abcd_box_path, abcd_data_path, abcd_covid, "2_Data_Release/")
abcd_covid_r3_files_path = file.path(main_abcd_path, abcd_box_path, abcd_data_path, abcd_covid, "3_Data_Release/")
additional_files_path = file.path(main_abcd_path, abcd_box_path, "Additional files/")
prs_files_path = file.path(main_abcd_path, prs_box_path)
p_factor_files_path = file.path(main_abcd_path, abcd_box_path, p_factor_box_path)
e_factor_files_path = file.path(main_abcd_path, abcd_box_path, e_factor_box_path)
genetic_files_path = file.path(main_abcd_path, abcd_box_path, abcd_data_path, "genetics/")
climate_mh_path = file.path(main_abcd_path, abcd_box_path, "Projects/climate_mental health/data")