...
climate_data_S1 <- climate_data %>% 
  # age-years at 2-year follow-up
  filter(eventname != "2_year_follow_up_y_arm_1") %>% 
  select(src_subject_id, sex_br, interview_age, race, dx90, site_id_l_br) %>% 
  left_join(
    climate_data %>%  filter(eventname == "2_year_follow_up_y_arm_1") %>% select(src_subject_id, interview_age)
  ) %>% 
  mutate(age_years = interview_age/12)

table1_vars <- c("age_years", "sex_br", "race", "dx90") #"race_white", "race_black", "race_aian", "race_nhpi",  "race_asian", "race_other", "race_mixed",
                 # "ethnicity_hisp", "non_hispanic_white"
table1_factors <- c("sex_br", "race")

write.csv(
  print(
    CreateTableOne(
      data = climate_data_S1 %>% distinct(src_subject_id, .keep_all = T),
      vars = table1_vars,
...
CreateTableOne(
  data = climate_data %>% distinct(src_subject_id, .keep_all = T),
  strata = "sites_sens",
  vars = "race",
  factorVars = "race"
)

# write.csv(print(CreateTableOne(
#   data = climate_data %>% distinct(src_subject_id, .keep_all = T),
#   strata = "site_id_l_br",
#   vars = "race",
#   factorVars = "race"
# )), "../outputs/race_by_sites.csv")
```
...
print(CreateTableOne(
  data = climate_data %>% distinct(src_subject_id, .keep_all = T) %>% filter(race %in% c("Non-Hispanic Black", "Non-Hispanic White")),
  strata = "race",
  vars = "dx90"
), pDigits = 5)

print(CreateTableOne(
  data = climate_data %>% distinct(src_subject_id, .keep_all = T) %>% filter(race %in% c("Non-Hispanic Black", "Hispanic")),
  strata = "race",
  vars = "dx90"
), pDigits = 5)

print(CreateTableOne(
  data = climate_data %>% distinct(src_subject_id, .keep_all = T) %>% filter(race %in% c("Non-Hispanic White", "Hispanic")),
  strata = "race",
  vars = "dx90"
), pDigits = 5)
```
...
```{r}
# Get adjusted p-values for table S7
# Hispanic & Black 0.76, rest <.001
p_adj <- round(p.adjust(c(.76, .0001, .0001), method = "fdr"), 4)
# 0.7600 0.0002 0.0002
```
...
ggplot(plot_dat, aes(x = race, y = Mean, fill = race)) +  
  ...
  annotate("segment", x = 2, xend = 3, y = 7.5, yend = 7.5) +
